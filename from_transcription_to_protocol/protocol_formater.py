...
def create_informal_docx(yandex_gpt_answer: str, output_path: str, timestamps=False, context=False) -> None:
    ...
    Returns: None
    """
    with open(get_path(yandex_gpt_answer), 'r', encoding='utf-8') as file:
        data = json.load(file)

    # Копирование шаблона неофициального
    with open(get_path(template_path), 'rb') as src, open(get_path(output_path), 'wb') as dst:
        dst.write(src.read())

    font_name = "Century Gothic"

    doc = Document(get_path(output_path))
    ...
    for idx in range(len(data["Тезисы"])):
        ...
        if context:
            ...
            run_2 = list_paragraph_2.add_run(f"           Контекст обсуждения: {data['Тезисы'][idx]['Контекст обсуждения']}")
