...
async def callback_transcribtion(call: CallbackQuery):
    ...
    #core.transcribize_timestamps(FORMATTED_DIR, f"{RESULT_PATH}/{call.from_user.id}_{call.message.message_id}_transcirbtion")
    os.system("cp cringe.txt " + f"{RESULT_PATH}/{call.from_user.id}_{call.message.message_id}_transcribtion.txt")

    if result:
        media = [
            types.InputMediaDocument(media=types.FSInputFile(f"{RESULT_PATH}/{call.from_user.id}_{call.message.message_id}_transcribtion.txt"))
        ]
